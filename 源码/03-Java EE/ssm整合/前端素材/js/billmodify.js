...
$(function(){
	...
	productDesc.on("focus",function(){
		validateTip(productDesc.next(),{"color":"#666666"},"* 请输入商品简介",false);
	}).on("blur",function(){
		if(productDesc.val() != null && productDesc.val() != ""){
			validateTip(productDesc.next(),{"color":"green"},imgYes,true);
		}else{
			validateTip(productDesc.next(),{"color":"red"},imgNo+" 商品简介不能为空，请重新输入",false);
		}
	...
	backBtn.on("click",function(){
		...
		if(referer != undefined 
			...
			&& "null" != referer
			&& referer.length > 4){
		 window.location.href = path +"/jsp/queryBill";
		}else{
			history.back(-1);
		}
	});
